    ...
    public boolean onCommand(CommandSender sender, Command command, String string, String[] arg) {
        ...
        if (arg.length == 0) return (!(sender instanceof Player)) ?
                MessageHandler.createInstance().sendMessage(Message.USAGE_GAMEMODE,
                        s -> s.replace("%command%", string), sender) : GameModeHandler.createInstance().setSelfGameMode((Player) sender, GameMode.CREATIVE);
        // If arg[0] is specified. ex: /gmc Pvbble
        return GameModeHandler.createInstance().setTargetGameMode(sender, Bukkit.getServer().getPlayer(arg[0]), GameMode.CREATIVE);
    }
}